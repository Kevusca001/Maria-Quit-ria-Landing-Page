  ...
  return (
    <section className="py-12 md:py-32 bg-smoke relative border-y border-wine/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {reviews.map((review, index) => (
            <motion.div
              ...
              className="bg-deep-black p-10 rounded-sm border border-wine/20 relative shadow-[0_10px_30px_rgba(0,0,0,0.5)]"
            >
              <Quote className="absolute top-6 left-6 w-12 h-12 text-wine/20" />
              <p className="text-xl text-gray-300 font-serif italic mb-8 relative z-10 leading-relaxed">
                &quot;{review.text}&quot;
              </p>
              <div className="flex items-center">
                <div className="w-10 h-10 rounded-full bg-wine/30 flex items-center justify-center text-white font-bold mr-4">
        ...
        {/* Ficha Técnica Placeholder */}
        <motion.div
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          transition={{ duration: 1, delay: 0.8 }}
          className="mt-24 text-center border-t border-wine/20 pt-16"
        >
          ...
          <div className="flex flex-wrap justify-center gap-8 md:gap-16 text-gray-400 font-sans text-sm uppercase tracking-widest">
            <div>
              <span className="block text-wine mb-2">Direção</span>
              <span>[Nome da Direção]</span>
            </div>
            <div>
              <span className="block text-wine mb-2">Elenco</span>
              <span>[Nome do Elenco]</span>
            </div>
            <div>
              <span className="block text-wine mb-2">Teatro</span>
